...
def sample_history(action_registry):
    ...
    return PhoneAgentHistoryList(history=histories)


def test_last_model_output(sample_history: PhoneAgentHistoryList):
    last_output = sample_history.last_action()
    print(last_output)
    assert last_output == {"done": {"text": "Task completed"}}


def test_get_errors(sample_history: PhoneAgentHistoryList):
    errors = sample_history.errors()
    assert len(errors) == 1
    assert errors[0] == "Failed to extract completely"


def test_final_result(sample_history: PhoneAgentHistoryList):
    assert sample_history.final_result() == "Task completed"


def test_is_done(sample_history: PhoneAgentHistoryList):
    assert sample_history.is_done() is True


def test_urls(sample_history: PhoneAgentHistoryList):
    urls = sample_history.urls()
    assert "https://example.com" in urls
    assert "https://example.com/page2" in urls


def test_all_screenshots(sample_history: PhoneAgentHistoryList):
    screenshots = sample_history.screenshots()
    assert len(screenshots) == 3
    assert screenshots == ["screenshot1.png", "screenshot2.png", "screenshot3.png"]


def test_all_model_outputs(sample_history: PhoneAgentHistoryList):
...
def test_all_model_outputs_filtered(sample_history: PhoneAgentHistoryList):
...
def test_empty_history():
    empty_history = PhoneAgentHistoryList(history=[])
