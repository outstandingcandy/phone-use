...
from browser_use.mobile_agent.gif import create_history_gif
from browser_use.mobile_agent.memory.service import Memory, MemorySettings
from browser_use.mobile_agent.message_manager.service import (
    MessageManager,
    MessageManagerSettings,
)
from browser_use.mobile_agent.message_manager.utils import (
    convert_input_messages,
    extract_json_from_model_output,
    save_conversation,
)
from browser_use.mobile_agent.prompts import (
    AgentMessagePrompt,
    PlannerPrompt,
    SystemPrompt,
)
...
from browser_use.phone.phone import Phone
from browser_use.phone.context import PhoneContext
from browser_use.phone.views import PhoneState, PhoneStateHistory
from browser_use.phone_controller.registry.views import PhoneActionModel
from browser_use.phone_controller.service import Controller
...
from browser_use.phone.utils import take_phone_screenshot, parse_phone_screenshot
...
def log_response(response: PhoneAgentOutput) -> None:
...
AgentHookFunc = Callable[["MobileAgent"], Awaitable[None]]
...
class Agent(Generic[Context]):
    ...
    def __init__(
        ...
        phone_context: PhoneContext | None = None,
        ...
        register_new_step_callback: Union[
            Callable[["PhoneState", "PhoneAgentOutput", int], None],  # Sync callback
            Callable[
                ["PhoneState", "PhoneAgentOutput", int], Awaitable[None]
            ],  # Async callback
            None,
        ] = None,
        register_done_callback: Union[
            Callable[["PhoneAgentHistoryList"], Awaitable[None]],  # Async Callback
            Callable[["PhoneAgentHistoryList"], None],  # Sync Callback
            None,
        ] = None,
        ...
        self.version = "iPhone14 Plus"
        self.source = "Apple"

        self.settings = PhoneAgentSettings(
        ...
        # Initialize state
        self.state = injected_agent_state or PhoneAgentState()
        ...
        self.phone_context = phone_context or PhoneContext(self.phone)
    ...
    def _setup_action_models(self) -> None:
        ...
        self.PhoneActionModel = self.controller.registry.create_action_model()
        # Create output model with the dynamic actions
        self.PhoneAgentOutput = PhoneAgentOutput.type_with_custom_actions(
            self.PhoneActionModel
        )
        ...
        self.DoneAgentOutput = PhoneAgentOutput.type_with_custom_actions(
    ...
    async def step(self, step_info: Optional[PhoneAgentStepInfo] = None) -> None:
        """Execute one step of the task"""
        logger.info(f"📍 Step {self.state.n_steps}")
        state = None
        model_output = None
        result: list[PhoneActionResult] = []
        ...
        try:
            ...
            if step_info and step_info.is_last_step():
                ...
                self.PhoneAgentOutput = self.DoneAgentOutput
            ...
            result: list[PhoneActionResult] = await self.multi_act(model_output.action)
            ...
            self.state.last_result = [
                P(error="The agent was paused with Ctrl+C", include_in_memory=False)
            ]
    ...
    async def _handle_step_error(self, error: Exception) -> list[PhoneActionResult]:
        """Handle all types of errors that can occur during a step"""
        include_trace = logger.isEnabledFor(logging.DEBUG)
        error_msg = PhoneAgentError.format_error(error, include_trace=include_trace)
        prefix = f"❌ Result failed {self.state.consecutive_failures + 1}/{self.settings.max_failures} times:\n "
        self.state.consecutive_failures += 1

        if "Phone closed" in error_msg:
            logger.error("❌  Phone is closed or disconnected, unable to proceed")
            return [
                PhoneActionResult(
                    error="Phone closed or disconnected, unable to proceed",
                    include_in_memory=False,
                )
            ]
        ...
        return [PhoneActionResult(error=error_msg, include_in_memory=True)]

    def _make_history_item(
        self,
        model_output: PhoneAgentOutput | None,
        state: PhoneState,
        result: list[PhoneActionResult],
        metadata: Optional[PhoneStepMetadata] = None,
    ) -> None:
        """Create and store history item"""

        state_history = PhoneStateHistory(
            screenshot=state.screenshot,
        )

        history_item = PhoneAgentHistory(
    ...
    async def get_next_action(
        ...
        if self.tool_calling_method == "raw":
            ...
            try:
                ...
                parsed = self.PhoneAgentOutput(**parsed_json)
            ...
            structured_llm = self.llm.with_structured_output(
                self.PhoneAgentOutput, include_raw=True
            )
            try:
                response: dict[str, Any] = await structured_llm.ainvoke(input_messages)  # type: ignore
                parsed: PhoneAgentOutput | None = response["parsed"]
            ...
            structured_llm = self.llm.with_structured_output(
                self.PhoneAgentOutput, include_raw=True, method=self.tool_calling_method
            )
            response: dict[str, Any] = await structured_llm.ainvoke(input_messages)  # type: ignore
        ...
        if response.get("parsing_error") and "raw" in response:
            ...
            if hasattr(raw_msg, "tool_calls") and raw_msg.tool_calls:
                # Convert tool calls to PhoneAgentOutput format
                ...
                parsed = self.PhoneAgentOutput(
                    current_state=current_state,
                    action=[self.PhoneActionModel(**action)],
                )
            else:
                parsed = None
        else:
    ...
    async def run(
        ...
        assert (
            self.llm._verified_api_keys or SKIP_LLM_API_KEY_VERIFICATION
        ...
        try:
            ...
            for step in range(max_steps):
                ...
                step_info = PhoneAgentStepInfo(step_number=step, max_steps=max_steps)
    ...
    async def multi_act(
        self,
        actions: list[PhoneActionModel],
        check_for_new_elements: bool = True,
    ) -> list[PhoneActionResult]:
        """Execute multiple actions"""
        results = []

        # cached_selector_map = await self.phone_context.get_selector_map()
        # cached_path_hashes = set(
        #     e.hash.branch_path_hash for e in cached_selector_map.values()
        # )

        # await self.phone_context.remove_highlights()

        for i, action in enumerate(actions):
            if action.get_index() is not None and i != 0:
                new_state = await self.phone_context.get_state()
                ...
                if orig_target_hash != new_target_hash:
                    ...
                    logger.info(msg)
                    results.append(
                        PhoneActionResult(extracted_content=msg, include_in_memory=True)
                    )
                    break
            ...
            try:
                ...
                await asyncio.sleep(self.phone_context.config.wait_between_actions)
    ...
    async def rerun_history(
        ...
        for i, history_item in enumerate(history.history):
            ...
            if (
                ...
                results.append(PhoneActionResult(error="No action to replay"))
                continue
                ...
                try:
                    ...
                    if retry_count == max_retries:
                        ...
                        if not skip_failures:
                            results.append(PhoneActionResult(error=error_msg))
    ...
    async def _update_action_indices(
        ...
        action: PhoneActionModel,  # Type this properly based on your action model
        current_state: PhoneState,
    ) -> Optional[PhoneActionModel]:
        """
    ...
    async def load_and_rerun(
        ...
        history = PhoneAgentHistoryList.load_from_file(
            history_file, self.PhoneAgentOutput
        )
    ...
    def resume(self) -> None:
        ...
        if self.phone:
            logger.info("🌎 Restarting/reconnecting to phone...")
            loop = asyncio.get_event_loop()
    ...
    def _convert_initial_actions(
    ...
    ) -> List[PhoneActionModel]:
        """Convert dictionary-based actions to PhoneActionModel instances"""
        converted_actions = []
        action_model = self.PhoneActionModel
        for action_dict in actions:
            ...
            # Create PhoneActionModel instance with the validated parameters
            action_model = self.PhoneActionModel(**{action_name: validated_params})
    ...
    async def _run_planner(self) -> Optional[str]:
        ...
        page = await self.phone_context.screenshot
    ...
    async def _update_action_models_for_page(self, page) -> None:
        ...
        self.PhoneActionModel = self.controller.registry.create_action_model(page=page)
