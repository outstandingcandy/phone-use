...
class SearchWechatAction(BaseModel):
    query: str
...
class TouchScreenAction(BaseModel):
    top: float
    left: float
    right: float
    bottom: float
    x_offset: Optional[float] = 0.0
    y_offset: Optional[float] = 25.0


class GetTouchableElementAction(BaseModel):
    encoded_image: str
...
class ClickKeyboardAction(BaseModel):
    key_pos_list: list[tuple[int, int]]
